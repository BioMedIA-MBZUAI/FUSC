...
def get_criterion(p):
    ...
    elif p['criterion'] == 'fusc':
        from losses.losses import FUSCLoss
        criterion = FUSCLoss(**p['criterion_kwargs'])
...
def get_model(p, pretrain_path=None):
    ...
    if p['backbone'] == 'resnet18':
        if p['train_db_name'] in ['cifar-10', 'cifar-20']:
            ...
            backbone = resnet18()
        elif p['train_db_name'] == 'US':
            from models.resnet_US import resnet18
            backbone = resnet18()
        else:
            raise NotImplementedError
    ...
    elif p['setup'] in ['fusc', 'selflabel']:
    ...
    if pretrain_path is not None and os.path.exists(pretrain_path):
        ...
        if p['setup'] == 'fusc': # Weights are supposed to be transfered from contrastive training
        ...
        elif p['setup'] == 'selflabel': # Weights are supposed to be transfered from fusc 
...
def get_train_dataset(p, transform, to_augmented_dataset=False,
    ...
    if p['train_db_name'] == 'cifar-10':
        ...
        from data.US import US
        dataset = US( p['csv_path'],p['img_folder_path'], transform=transform)
    else:
