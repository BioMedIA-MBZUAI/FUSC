...
from utils.evaluate_utils import get_predictions, fusc_evaluate, hungarian_evaluate
from utils.train_utils import fusc_train

FLAGS = argparse.ArgumentParser(description='FUSC Loss')
...
def main():
    ...
    if p['update_cluster_head_only']:
        print(colored('WARNING: FUSC will only update the cluster head', 'red'))
    ...
    # Checkpoint
    if os.path.exists(p['fusc_checkpoint']):
        print(colored('Restart from checkpoint {}'.format(p['fusc_checkpoint']), 'blue'))
        checkpoint = torch.load(p['fusc_checkpoint'], map_location='cpu')
        ...
        print(colored('No checkpoint file at {}'.format(p['fusc_checkpoint']), 'blue'))
        start_epoch = 0
        best_loss = 1e4
        best_loss_head = None
    ...
    for epoch in range(start_epoch, p['epochs']):
        ...
        # Train
        print('Train ...')
        fusc_train(train_dataloader, model, criterion, optimizer, epoch, p['update_cluster_head_only'])

        # Evaluate 
        print('Make prediction on validation set ...')
        predictions = get_predictions(p, val_dataloader, model)

        print('Evaluate based on FUSC loss ...')
        fusc_stats = fusc_evaluate(predictions)
        print(fusc_stats)
        lowest_loss_head = fusc_stats['lowest_loss_head']
        lowest_loss = fusc_stats['lowest_loss']
        ...
        if lowest_loss < best_loss:
            ...
            torch.save({'model': model.module.state_dict(), 'head': best_loss_head}, p['fusc_model'])
    ...
    print(colored('Evaluate best model based on fusc metric at the end', 'blue'))
    model_checkpoint = torch.load(p['fusc_model'], map_location='cpu')
    ...
    clustering_stats = hungarian_evaluate(model_checkpoint['head'], predictions, 
                            ...
                            confusion_matrix_file=os.path.join(p['fusc_dir'], 'confusion_matrix.png'))
    print(clustering_stats)         
